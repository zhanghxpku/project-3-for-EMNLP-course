    ...
    def build_graph(self, inputs, mode):
        ...
        if config.use_lstm:
            ...
            if not config.use_word_pretrain_emb:
                word_emb_layer = layers.EmbeddingLayer(config.vocab_size_emb, config.word_emb_size, name='word_emb')
            else:
                word_emb_layer = layers.InitializedEmbeddingLayer(config.vocab_size_emb, config.word_emb_size, config.word2vec_emb, trainable=config.word_emb_finetune, name='word_emb')
            
            # [batch_size, max_len, emb_size]
            word_emb_lstm = word_emb_layer(inputs['word'], zero_forward=True)
#    #        print 'word_emb', word_emb
#            nwords = tf.count_nonzero(inputs['word_emb'], axis=-1, dtype=tf.int32)
#    #        print 'nwords', nwords
    
            tag_emb_lstm = self.relation_emb_layer_word
#            # [relation_size, relation_max_len, emb_size]
#            nwords_tag = tf.count_nonzero(tag_emb_lstm, axis=-1, dtype=tf.int32)
#    #        print 'nwords_tag', nwords_tag
            tag_emb_lstm = word_emb_layer(tag_emb_lstm, zero_forward=True)
#    #        print 'tag_emb', tag_emb
#    
#            word_emb_lstm = tf.transpose(word_emb_lstm, perm=[1, 0, 2])
#            tag_emb_lstm = tf.transpose(tag_emb_lstm, perm=[1, 0, 2])
#            # Bi-LSTM
...
#                output_fw, _ = lstm_cell_fw(word_emb_lstm, dtype=tf.float32, sequence_length=nwords)
#                output_fw = tf.layers.dropout(output_fw, rate=config.dropout_rate, training=training)
#                output_fw_tag, _ = lstm_cell_fw(tag_emb_lstm, dtype=tf.float32, sequence_length=nwords_tag)
#                output_fw_tag = tf.layers.dropout(output_fw_tag, rate=config.dropout_rate, training=training)
#            with tf.variable_scope('lstm2'):
#                output_bw, _ = lstm_cell_bw(word_emb_lstm, dtype=tf.float32, sequence_length=nwords)
#                output_bw = tf.layers.dropout(output_bw, rate=config.dropout_rate, training=training)
#                output_bw_tag, _ = lstm_cell_bw(tag_emb_lstm, dtype=tf.float32, sequence_length=nwords_tag)
#                output_bw_tag = tf.layers.dropout(output_bw_tag, rate=config.dropout_rate, training=training)
#            h_word_emb = tf.concat([output_fw, output_bw], axis=-1)
#            h_word_emb = tf.transpose(h_word_emb, perm=[1, 0, 2])
#            h_tag_emb = tf.concat([output_fw_tag, output_bw_tag], axis=-1)
#            h_tag_emb = tf.transpose(h_tag_emb, perm=[1, 0, 2])
            h_word_emb = word_emb_lstm
            h_tag_emb = tag_emb_lstm
